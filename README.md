...
- 内置 10 个中文攻防场景，包含 5 个正常任务和 5 个攻击任务。
- 支持间接提示词注入、工具输出投毒、数据外泄、多步工具链和越权操作。
...
以下结果由仓库内置的 10 个固定场景和本地规则引擎生成，可以通过命令重复验证：
...
`send_email`、`export_data`、`delete_file` 和 `read_secret` 等工具具有较高风险。严格模式会暂停这些调用，等待人工批准。
...
│   └── tools.py                    # 内存虚拟工具箱
├── data/scenarios.json             # 10 个中文攻防场景
...
8. `app.py`：最后阅读界面如何展示这些数据。

更细的学习任务见 [docs/学习指南.md](docs/学习指南.md)。
...
> 使用 Python、Streamlit 构建面向工具调用 Agent 的提示词注入攻防实验平台，设计 10 个可复现的正常与攻击场景，实现注入检测、指令隔离、工具白名单、敏感数据防泄漏和人工审批机制；在内置测试集上将攻击成功率从 100% 降至 0%，同时保持平衡模式任务成功率为 100%，并通过 GitHub Actions 完成自动化测试和静态检查。
...
> 我做的是一个大模型 Agent 安全评测平台。普通大模型应用只生成文本，但 Agent 会调用邮件、文件和数据导出等工具，所以不可信文档里的提示词注入可能造成真实副作用。我设计了 10 个可复现的中文场景，并把模型决策、工具权限、防御策略和评测指标拆成独立模块。系统支持五类防御，能够记录每一次工具调用和阻断原因。在内置测试集上，基线攻击成功率是 100%，平衡防御将它降到 0%，同时保持正常任务成功率 100%。严格模式虽然也能阻断攻击，但会误拦截需要审批的正常邮件任务，这说明 Agent 安全需要同时衡量安全性和可用性。
...
本地攻击决策是规则驱动，真实模型的工具来源归因也使用场景标签进行简化。下一步应加入多模型重复采样、语义注入检测、真实 MCP 工具协议和更细的信息流追踪。
...
- 将虚拟工具改造成标准 MCP Server，并实现工具能力动态发现；
